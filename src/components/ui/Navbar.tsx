    ...
    <nav className={navClasses}>
      <div className="container mx-auto px-6 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center space-x-8">
          ...
          <Link
            href="/#contact"
            className="px-6 py-2 bg-accent text-background text-xs font-bold uppercase tracking-widest rounded-full hover:bg-accent/90 transition-all"
          >
            Hire Me
          </Link>
        </div>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="fixed inset-0 top-[72px] bg-background z-40 md:hidden flex flex-col items-center justify-center space-y-8"
          >
            {navLinks.map((link) => (
              <Link
                key={link.name}
            ...
            <Link
              href="/#contact"
              className="px-8 py-3 bg-accent text-background font-bold uppercase tracking-widest rounded-full"
              onClick={() => setIsOpen(false)}
            >
              Hire Me
            </Link>
