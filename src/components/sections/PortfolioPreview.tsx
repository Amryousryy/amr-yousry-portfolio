    ...
    const fetchFeatured = async () => {
      try {
        ...
        if (data) {
          ...
          console.warn("PortfolioPreview: Database fetch failed, using fallbacks if available.", error);
        }
      } catch (err) {
    ...
    <section className="py-24 bg-background border-t border-primary/10">
      <div className="container mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-16 gap-6">
          <div>
            <h2 className="pixel-text text-accent text-sm mb-4 tracking-[0.3em]">Featured Work</h2>
          ...
          {!loading ? (
                ...
                <Link href={['/projects', project._id].join('/')} className="block">
                  <div className="relative aspect-[4/5] overflow-hidden pixel-border mb-6">
                    ...
                    <div className="absolute inset-0 bg-primary/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                      <span className="pixel-text text-background bg-accent px-6 py-2 font-bold transform -rotate-2">
                        View Project
                      </span>
                    </div>
                  </div>

                  <span className="text-xs text-accent font-bold uppercase tracking-widest block mb-2 pixel-text">
                    {project.category}
                  </span>
            ...
            [1, 2, 3].map((i) => (
              <div key={i} className="animate-pulse">
                <div className="aspect-[4/5] bg-primary/10 mb-6 pixel-border" />
                <div className="h-4 bg-primary/10 w-24 mb-4" />
                <div className="h-8 bg-primary/10 w-full" />
              </div>
            ))
          )}
        </div>
